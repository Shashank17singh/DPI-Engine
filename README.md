...
# Run against the included sample capture
...
│   ├── main_working.cpp       # ★ SIMPLE VERSION ★ (builds: dpi_simple)
│   └── dpi_mt.cpp             # ★ MULTI-THREADED VERSION ★ (builds: dpi_engine)
│
├── legacy/                     # Earlier iterations, kept for reference only
│   ├── src/                   # main.cpp, main_simple.cpp, main_dpi.cpp,
│   │                          # dpi_engine.cpp, and the modular
│   │                          # rule_manager / connection_tracker /
│   │                          # load_balancer / fast_path prototype
│   └── include/               # Matching headers for the files above
│
├── generate_test_pcap.py      # Creates test data
├── test_dpi.pcap              # Sample capture with various traffic
├── CMakeLists.txt             # Build config (builds dpi_simple + dpi_engine)
└── README.md                  # This file!
```

> **Note:** `legacy/` holds earlier prototypes from development (an object-oriented
> orchestrator design and a modular thread-pool design) that were superseded by
> `main_working.cpp` and `dpi_mt.cpp`. They aren't part of the CMake build but are
> kept in the repo since they show the design evolution.
...
cmake -B build -DCMAKE_BUILD_TYPE=Release
cmake --build build
# Produces build/dpi_simple and build/dpi_engine
```
